...
<?php include_once "Sitessmryinfo.php" ?>
<?php
...
$Sites_summary = NULL; // Initialize page object first

class crSites_summary extends crSites {
	...
	function __construct() {
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Sites', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fSitessummary";
	}
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_Sites\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_Sites',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fSitessummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fSitessummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fSitessummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_Sites_name"])) {
			$this->LoadSelectionFromSession('name');
		} elseif (@$_SESSION["sel_Sites_name"] == EWR_INIT_VALUE) { // Select all
			$this->name->SelectionList = "";
		}

		// Get sitedate selected values
		if (is_array(@$_SESSION["sel_Sites_sitedate"])) {
			$this->LoadSelectionFromSession('sitedate');
		} elseif (@$_SESSION["sel_Sites_sitedate"] == EWR_INIT_VALUE) { // Select all
			$this->sitedate->SelectionList = "";
		}

		// Get units selected values
		if (is_array(@$_SESSION["sel_Sites_units"])) {
			$this->LoadSelectionFromSession('units');
		} elseif (@$_SESSION["sel_Sites_units"] == EWR_INIT_VALUE) { // Select all
			$this->units->SelectionList = "";
		}

		// Get rate selected values
		if (is_array(@$_SESSION["sel_Sites_rate"])) {
			$this->LoadSelectionFromSession('rate');
		} elseif (@$_SESSION["sel_Sites_rate"] == EWR_INIT_VALUE) { // Select all
			$this->rate->SelectionList = "";
		}
	}
	...
	function GetExtendedFilter() {
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_Sites_name'] = ($this->name->SelectionList == "") ? EWR_INIT_VALUE : $this->name->SelectionList;

			// Field sitedate
			$sWrk = "";
			$this->BuildExtendedFilter($this->sitedate, $sWrk);
			ewr_LoadSelectionFromFilter($this->sitedate, $sWrk, $this->sitedate->SelectionList);
			$_SESSION['sel_Sites_sitedate'] = ($this->sitedate->SelectionList == "") ? EWR_INIT_VALUE : $this->sitedate->SelectionList;

			// Field units
			$sWrk = "";
			$this->BuildExtendedFilter($this->units, $sWrk);
			ewr_LoadSelectionFromFilter($this->units, $sWrk, $this->units->SelectionList);
			$_SESSION['sel_Sites_units'] = ($this->units->SelectionList == "") ? EWR_INIT_VALUE : $this->units->SelectionList;

			// Field rate
			$sWrk = "";
			$this->BuildExtendedFilter($this->rate, $sWrk);
			ewr_LoadSelectionFromFilter($this->rate, $sWrk, $this->rate->SelectionList);
			$_SESSION['sel_Sites_rate'] = ($this->rate->SelectionList == "") ? EWR_INIT_VALUE : $this->rate->SelectionList;
		}
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_Sites_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Sites_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_Sites_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Sites_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_Sites_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_Sites_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_Sites_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_Sites_' . $parm] = $sv;
		$_SESSION['so_Sites_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_Sites_' . $parm] = $sv1;
		$_SESSION['so_Sites_' . $parm] = $so1;
		$_SESSION['sc_Sites_' . $parm] = $sc;
		$_SESSION['sv2_Sites_' . $parm] = $sv2;
		$_SESSION['so2_Sites_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_Sites_$parm"] = "";
		$_SESSION["rf_Sites_$parm"] = "";
		$_SESSION["rt_Sites_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_Sites_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_Sites_$parm"];
		$fld->RangeTo = @$_SESSION["rt_Sites_$parm"];
	}
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_sitedate", $filter)) {
			...
			$_SESSION["sel_Sites_sitedate"] = $sWrk;
...
// Create page object
if (!isset($Sites_summary)) $Sites_summary = new crSites_summary();
...
// Create page object
var Sites_summary = new ewr_Page("Sites_summary");

// Page properties
Sites_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = Sites_summary.PageID;

// Extend page with Chart_Rendering function
Sites_summary.Chart_Rendering = 
...
// Form object
var CurrentForm = fSitessummary = new ewr_Form("fSitessummary");

// Validate method
fSitessummary.Validate = function() {
...
fSitessummary.Form_CustomValidate = 
...
fSitessummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fSitessummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fSitessummary" id="fSitessummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fSitessummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="Sites" data-field="x_name" id="sv_name" name="sv_name" size="30" maxlength="111" placeholder="<?php echo $Page->name->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->name->SearchValue) ?>"<?php echo $Page->name->EditAttributes() ?>>
</span>
...
<?php ewr_PrependClass($Page->name->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="Sites" data-field="x_name" id="sv2_name" name="sv2_name" size="30" maxlength="111" placeholder="<?php echo $Page->name->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->name->SearchValue2) ?>"<?php echo $Page->name->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="Sites" data-field="x_sitedate" id="sv_sitedate" name="sv_sitedate" placeholder="<?php echo $Page->sitedate->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->sitedate->SearchValue) ?>"<?php echo $Page->sitedate->EditAttributes() ?>>
</span>
...
<?php ewr_PrependClass($Page->sitedate->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="Sites" data-field="x_sitedate" id="sv2_sitedate" name="sv2_sitedate" placeholder="<?php echo $Page->sitedate->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->sitedate->SearchValue2) ?>"<?php echo $Page->sitedate->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="Sites" data-field="x_units" id="sv_units" name="sv_units" size="30" placeholder="<?php echo $Page->units->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->units->SearchValue) ?>"<?php echo $Page->units->EditAttributes() ?>>
</span>
...
<?php ewr_PrependClass($Page->units->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="Sites" data-field="x_units" id="sv2_units" name="sv2_units" size="30" placeholder="<?php echo $Page->units->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->units->SearchValue2) ?>"<?php echo $Page->units->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="Sites" data-field="x_rate" id="sv_rate" name="sv_rate" size="30" placeholder="<?php echo $Page->rate->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->rate->SearchValue) ?>"<?php echo $Page->rate->EditAttributes() ?>>
</span>
...
<?php ewr_PrependClass($Page->rate->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="Sites" data-field="x_rate" id="sv2_rate" name="sv2_rate" size="30" placeholder="<?php echo $Page->rate->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->rate->SearchValue2) ?>"<?php echo $Page->rate->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fSitessummary.Init();
fSitessummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "Sitessmrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<!-- Report grid (begin) -->
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="name"><div class="Sites_name"><span class="ewTableHeaderCaption"><?php echo $Page->name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="name">
<?php if ($Page->SortUrl($Page->name) == "") { ?>
		<div class="ewTableHeaderBtn Sites_name">
			<span class="ewTableHeaderCaption"><?php echo $Page->name->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Sites_name', false, '<?php echo $Page->name->RangeFrom; ?>', '<?php echo $Page->name->RangeTo; ?>');" id="x_name<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Sites_name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->name->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="sitedate"><div class="Sites_sitedate"><span class="ewTableHeaderCaption"><?php echo $Page->sitedate->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="sitedate">
<?php if ($Page->SortUrl($Page->sitedate) == "") { ?>
		<div class="ewTableHeaderBtn Sites_sitedate">
			<span class="ewTableHeaderCaption"><?php echo $Page->sitedate->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Sites_sitedate', false, '<?php echo $Page->sitedate->RangeFrom; ?>', '<?php echo $Page->sitedate->RangeTo; ?>');" id="x_sitedate<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Sites_sitedate" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->sitedate) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->sitedate->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="units"><div class="Sites_units"><span class="ewTableHeaderCaption"><?php echo $Page->units->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="units">
<?php if ($Page->SortUrl($Page->units) == "") { ?>
		<div class="ewTableHeaderBtn Sites_units">
			<span class="ewTableHeaderCaption"><?php echo $Page->units->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Sites_units', false, '<?php echo $Page->units->RangeFrom; ?>', '<?php echo $Page->units->RangeTo; ?>');" id="x_units<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Sites_units" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->units) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->units->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="rate"><div class="Sites_rate"><span class="ewTableHeaderCaption"><?php echo $Page->rate->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="rate">
<?php if ($Page->SortUrl($Page->rate) == "") { ?>
		<div class="ewTableHeaderBtn Sites_rate">
			<span class="ewTableHeaderCaption"><?php echo $Page->rate->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Sites_rate', false, '<?php echo $Page->rate->RangeFrom; ?>', '<?php echo $Page->rate->RangeTo; ?>');" id="x_rate<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Sites_rate" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->rate) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->rate->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Sites_name"<?php echo $Page->name->ViewAttributes() ?>><?php echo $Page->name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->sitedate->Visible) { ?>
		<td data-field="sitedate"<?php echo $Page->sitedate->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Sites_sitedate"<?php echo $Page->sitedate->ViewAttributes() ?>><?php echo $Page->sitedate->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->units->Visible) { ?>
		<td data-field="units"<?php echo $Page->units->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Sites_units"<?php echo $Page->units->ViewAttributes() ?>><?php echo $Page->units->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->rate->Visible) { ?>
		<td data-field="rate"<?php echo $Page->rate->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Sites_rate"<?php echo $Page->rate->ViewAttributes() ?>><?php echo $Page->rate->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
